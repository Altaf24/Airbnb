...
//             <div className='w-[90%] min-h-[80px]  border-b-[1px] border-[#dcdcdc]  flex items-center justify-between md:px-[40px] '>
    ...
    return (
        ...
        <div className='fixed top-0 bg-[white] z-[20] w-full'>
            <div className='w-full min-h-[80px] border-b-[1px] border-[#dcdcdc] flex items-center justify-between px-[20px] md:px-[40px]'>
                ...
                <div className='w-[35%] max-w-[400px] relative hidden md:block mx-4'>
                    <input 
                        type="text" 
                        className='w-full px-[30px] py-[10px] border-[2px] border-[#bdbaba] outline-none overflow-auto rounded-[30px] text-[17px]' 
                ...
                {/* Right side - List your home + Profile button */}
                <div className='flex items-center justify-end gap-[15px] relative ml-auto'>
                    <span 
                        className='text-[16px] md:text-[18px] cursor-pointer rounded-[50px] hover:bg-[#ded9d9] px-[12px] py-[8px] whitespace-nowrap font-medium transition-colors duration-200' 
                        onClick={()=>navigate("/listingpage1")}
                    >
                        List your home
                    </span>
                    <button 
                        className='px-[15px] md:px-[20px] py-[10px] flex items-center justify-center gap-[8px] border-[1px] border-[#8d8c8c] rounded-[50px] hover:shadow-lg transition-shadow duration-200 flex-shrink-0' 
                        onClick={()=>setShowpopup(prev =>!prev)}
                    >
                        <span><GiHamburgerMenu className='w-[18px] h-[18px] md:w-[20px] md:h-[20px]' /></span>
                        {userData == null && <span><CgProfile className='w-[21px] h-[21px] md:w-[23px] md:h-[23px]' /></span>}
                        {userData != null && <span className='w-[28px] h-[28px] md:w-[30px] md:h-[30px] bg-[#080808] text-[white] rounded-full flex items-center justify-center text-[14px] md:text-[16px] font-medium'>{userData?.name.slice(0,1)}</span>}
                    </button>

                    {/* Popup Menu */}
                    {showpopup && (
                        <div className='w-[220px] h-[250px] absolute bg-slate-50 top-[110%] right-0 border-[1px] border-[#aaa9a9] z-10 rounded-lg shadow-lg'>
                            <ul className='w-[100%] h-[100%] text-[17px] flex items-start justify-around flex-col py-[10px]'>
                                {!userData && <li className='w-[100%] px-[15px] py-[10px] hover:bg-[#f4f3f3] cursor-pointer' onClick={()=>{navigate("/login");setShowpopup(false)}}>Login</li>}
                                {userData && <li className='w-[100%] px-[15px] py-[10px] hover:bg-[#f4f3f3] cursor-pointer' onClick={()=>{handleLogOut();setShowpopup(false)}}>Logout</li>}
                                <div className='w-[100%] h-[1px] bg-[#c1c0c0]'></div>
                ...
                {searchData?.length>0 && (
                    <div className='w-[100vw] h-[450px] flex flex-col gap-[20px] absolute top-[50%] overflow-auto left-[0] justify-start items-center'>
                        <div className='max-w-[700px] w-[100vw] h-[300px] overflow-hidden flex flex-col bg-[#fefdfd] p-[20px] rounded-lg border-[1px] border-[#a2a1a1] cursor-pointer'>
                            {
                                searchData.map((search)=>(
                                <div className='border-b border-[black] p-[10px]' onClick={()=>handleClick(search._id)} key={search._id}>
                                    {search.title} in {search.landMark},{search.city}
                                </div>
                                ))
                            }
